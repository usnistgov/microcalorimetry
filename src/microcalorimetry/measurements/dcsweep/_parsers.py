...
def colorbar(
    fig: plt.Figure, ax: plt.axes, values: np.array, map: str = 'jet', label: str = ''
):
...
def mean_settled_by_group(
    x: xr.DataArray, group_arr: xr.DataArray, values: xr.DataArray, window: float
    ...
    for osn in values:
        ...
        settled = group[group.time > group.time[-1] - window]
    ...
    output = []
    for i in [avg, std]:
        output.append(xr.DataArray(i, dims=('time'), coords={'time': times}))
...
def metered_to_linmeas(
    ...
    if specs is not None:
        ...
        meas.add_umech(
            ...
            pert,
            add_uid=True,
            category={'Type': 'B', 'Origin': origin + ' Traceability'},
        )
...
def parse_v0(
    ...
    heater_adjust_times = xr.DataArray(
        adjust_times, dims=('time'), coords={'time': heater_p.time}
    )
    heater_step_numbers = xr.DataArray(
        step_numbers, dims=('time'), coords={'time': heater_p.time}
    )

    # shift the sensor adjust times back by 1 sample inc case they are out
    # of sync by 1 sample
    sensor_adjust_times = heater_adjust_times.interp(
        time=e.time, method='nearest'
    ).shift(time=-2)
    sensor_adjust_times[-2:] = sensor_adjust_times[-3]
    sensor_step_numbers = heater_step_numbers.interp(
        time=e.time, method='nearest'
    ).shift(time=-2)
    ...
    e_ons, e_ons_std, e_on_samples = mean_settled_by_group(
        e, sensor_step_numbers, values=on_step_numbers, window=on_time_window
    )
    ...
    heater_v_ons, heater_v_ons_std, heater_v_ons_samples = mean_settled_by_group(
        heater_v, heater_step_numbers, values=on_step_numbers, window=on_time_window
    )

    # get the heater i ons
    heater_i_ons, heater_i_ons_std, heater_i_ons_samples = mean_settled_by_group(
        heater_i, heater_step_numbers, values=on_step_numbers, window=on_time_window
    )

    (p_ons_line,) = ax[1].plot(
        zero_hrs(heater_v_ons.time, t0), heater_v_ons * heater_i_ons * 1000, 'ko'
    )
    ...
    e_offs_mn, e_offs_std, e_off_samples = mean_settled_by_group(
        e, sensor_step_numbers, values=off_step_numbers, window=off_time_window
    )
    ...
    heater_v_offs, heater_v_offs_std, heater_v_offs_samples = mean_settled_by_group(
        heater_v, heater_step_numbers, values=off_step_numbers, window=off_time_window
    )

    # get the heater i offs
    heater_i_offs, heater_i_offs_std, heater_i_offs_samples = mean_settled_by_group(
        heater_i, heater_step_numbers, values=off_step_numbers, window=off_time_window
    )
    ...
    heater_i_specs = kspecs.DatasheetMeasureDCI(
        'K2450', serial='xxx', suppress_warnings=True
    )
    heater_v_specs = kspecs.DatasheetMeasureDCV(
        'K2450', serial='xxx', suppress_warnings=True
    )
    ...
    e_offs = metered_to_linmeas(
        'e_off', e_offs, specs=e_specs, umech_prefix='DC Sweep', origin='DC Sweep'
    )

    heater_v_ons = metered_to_linmeas(
        'heater_v_on',
        heater_v_ons,
        std=heater_v_ons_std,
        specs=heater_v_specs,
        umech_prefix='DC Sweep',
        origin='DC Sweep',
    )
    ...
    heater_i_ons = metered_to_linmeas(
        'heater_i_on',
        heater_i_ons,
        std=heater_i_ons_std,
        specs=heater_i_specs,
        umech_prefix='DC Sweep',
        origin='DC Sweep',
    )
...
def read_v1(
    runs: list[Path], thermopile_monitor: str, thermometer_monitor: str, heater: str
) -> DCCalibrationData:
    """
    ...
    def try_get(d, key, p: str):
        try:
            ...
            raise KeyError(
                f'expected {key} field in config file {p}. Not present, add manually.'
            ) from e
    ...
    for file in runs:
        ...
        data = {
            k: TimeSeries(v.t[:min_shape], v.values[:min_shape])
    ...
    data = xr.Dataset(
        data_vars=dict(
            ...
            heater_p=(
                ['time'],
                all_data[f'I_{heater} (A)'] * all_data[f'V_{heater} (V)'],
            ),
            heater_r=(
                ['time'],
                all_data[f'I_{heater} (A)'] / all_data[f'V_{heater} (V)'],
    ...
    if thermometer_monitor:
        data = data.assign(
            ...
            therm_p=(
                ['time'],
                all_data[f'V_{thermometer_monitor} (V)']
                * all_data[f'I_{thermometer_monitor} (A)'],
            ),
            therm_r=(
                ['time'],
                all_data[f'V_{thermometer_monitor} (V)']
    ...
    for i, crawl in enumerate(where_steps):
        ...
        follow = crawl
    data = data.assign(
        steps=xr.DataArray(steps, dims=('time',), coords=dict(time=data.time))
    )
...
def review_plot_v1(
    ...
    for var, ax, yscale, ylabel in zip(var_ls, axs, yscales, ylabels):
        ...
        if offs_samples is not None:
            ax.plot(
                zero(offs_samples.time),
                offs_samples[var] * yscale,
                'kx',
                label='Off Samples',
            )
...
def pick_window(x: DCCalibrationData, window: tuple[float, float]) -> DCCalibrationData:
    ...
    out = x.where(
        (x.adjust_time >= min_wait_time) & (x.adjust_time <= max_wait_time), drop=True
    )
...
def parse_v1(
    *args,
    ...
    offs_steps = data.where(
        (data.pwr_setting == 0) & (data.adjust_time >= off_min_wait), drop=True
    )
    offs_samples = offs_steps.groupby(offs_steps.steps).map(
        pick_window, args=(off_window,)
    )
    ...
    for v in offs_samples:
        offs[v] = offs_samples[v].interp(
            time=ons.time, kwargs=dict(fill_value=offs_samples[v].mean())
        )
    ...
    # color by approximate unique powers
    scatter = ax.scatter(
        data.e * 1e3, data.e / data.heater_p, c=data.env_temp, cmap=cmap
    )
    fig.colorbar(scatter, label='Ambient Temperature (°C)')
    ...
    # color by approximate unique powers
    scatter = ax.scatter(
        data.heater_p * 1e3, data.e / data.heater_p, c=data.env_temp, cmap=cmap
    )
    fig.colorbar(scatter, label='Ambient Temperature (°C)')
    ax.set_xlabel(r'$P_{dc}\:\left(\mathrm{mW}\right)$')
    ...
    # color by approximate unique powers
    scatter = ax.scatter(
        data.e * 1e3, data.e / data.heater_p, c=data.adjust_time / 3600, cmap=cmap
    )
    ...
    if thermometer_monitor:
        ...
        # color by approximate unique powers
        scatter = ax.scatter(
            data.e * 1e3, data.e / data.heater_p, c=data.therm_r / 1000, cmap=cmap
        )
    ...
    e_specs = import_spec(
        thermopile_monitor, 'DatasheetDCV', fallback_model='HP34420A'
    )('thermopile_monitor', serial='xxx', suppress_warnings=True)

    therm_i_specs = None
    therm_v_specs = None
    if thermometer_monitor:
        therm_i_specs = import_spec(
            thermometer_monitor, 'DatasheetMeasureDCI', fallback_model='K2450'
        )('thermometer_monitor_i', serial='xxx', suppress_warnings=True)
        therm_v_specs = import_spec(
            thermometer_monitor, 'DatasheetMeasureDCV', fallback_model='K2450'
