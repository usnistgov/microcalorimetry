...
from rmellipse import RMEMeas, AnnotatedArray
import numpy as np
import xarray as xr
from fnmatch import fnmatch

__all__ = ['categorize_by', 'DEFAULT_ORIGIN_FILTER']

"""Mapping of (keys) glob patterns of covcats origins that should be renamed to their values."""
...
def categorize_by(
    ...
    if len(unused_locs) > 0 and combine_uncategorized:
        new.append(
            sum_reexpand(
                meas.cov.sel(umech_id=unused_locs),
                uncategorized_name,
                nominal,
                deg,
                rad,
